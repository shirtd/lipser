...
RES=32
DSET='northwest' # 'rainier_sub'
DIR=os.path.join('data')
DDIR=os.path.join(DIR, DSET)
FILE=os.path.join(DDIR, f'{DSET}{RES}.csv')
JSON=os.path.join(DDIR, f'{DSET}{RES}.json')
SAMPLE_FILE= None
SUB_FILE= None
...
parser.add_argument('--json', default=JSON, help='surface config')
...
if __name__ == '__main__':
    ...
    kwargs = {  'surf'      : { 'zorder' : 0, 'alpha' : 0.5},
                ...
                'barcode'   : { 'cuts' : surf.cuts, 'colors' : surf.colors}}
    ...
    if args.barcode:
        ...
        plot_barcode(ax, surf_dgms[1], **kwargs['barcode'], lw=2)
        ...
        if args.contours:
            surf_plt = surf.plot_contours(ax, surf.cuts, surf.colors, args.save, args.dir, args.dpi)
        # elif not args.nosurf:
        #     surf_plt = surf.plot(ax, surf.cuts, COLORS, **kwargs['surf'])
        surf_plt = plot_surface(ax, surf, surf.cuts, surf.colors, zorder=0, alpha=0.5)
        plt.show()
